...
def ptrace_legacy(rho_data, rho_dims, rho_shape, _sel):
    """
    ...
    cdef size_t mm, ii
    cdef int _tmp
    cdef cnp.ndarray[int, ndim=1, mode='c'] drho = np.asarray(rho_dims[0], dtype=np.int32).ravel()
    ...
    rhdata = zcsr_mult(perm, zcsr_reshape(rho_data, np.prod(rho_shape), 1), sorted=0)
    rho1_data = zcsr_reshape(rhdata, M, M)
    dims_kept0 = np.asarray(rho_dims[0], dtype=np.int32).take(sel)
...
#         factor_tensor = factor_tensor * drho[ii]
#         if _in(ii, _sel):
#             tensor_table[1, ii] = factor_keep
#             factor_keep = factor_keep * drho[ii]
#         else:
#             tensor_table[2, ii] = factor_trace
#             factor_trace = factor_trace * drho[ii]
...
# @cython.wraparound(False)
# @cython.cdivision(True)
# def cy_ptrace_mq(object rho, sel): # work for N<= 26 on 16G Ram
#     cdef int[::1] _sel
#     cdef object _oper
#     cdef size_t ii
#     cdef size_t factor_keep = 1, factor_trace = 1, factor_tensor = 1
#     cdef cnp.ndarray[int, ndim=1, mode='c'] drho = np.asarray(rho.dims, dtype=np.int32).ravel()
#     cdef int num_dims = drho.shape[0]
#     cdef int[:, ::1] tensor_table = np.zeros((3, num_dims), dtype=np.int32)
...
#     # Try to evaluate how sparse the result will be.
#     if factor_keep*factor_keep > _oper.nnz:
#         return MyQobj(csr2fast(_ptrace_core_sp(_oper, tensor_table, factor_keep)), dims = rho1_dims[0])
#     else:
#         return MyQobj(csr2fast(_ptrace_core_dense(_oper, tensor_table, factor_keep)), dims = rho1_dims[0])
...
cdef object _ptrace_core_sp(rho, int[:, ::1] tensor_table, int num_sel_dims):
    ...
    return spar.coo_matrix((new_data, [new_row, new_col]),
...
cdef object _ptrace_core_dense(rho, int[:, ::1] tensor_table, int num_sel_dims):
    ...
    for ii in range(nrow):
        for jj in range(ptr[ii], ptr[ii+1]):
            ...
            _i2_k_t(ii, tensor_table, pos_r)
            if pos_c[1] == pos_r[1]:
                data_mat[pos_r[0], pos_c[0]] = data_mat[pos_r[0], pos_c[0]]+ data[jj]

    return spar.coo_matrix(data_mat).tocsr()